...
//    glVertexAttribIPointer(BONE_ID_LOCATION, 4, GL_INT, sizeof(VertexBoneData), (const GLvoid*)0);
	// use unsigned int because the id can't be negative (size issues??)
	glVertexAttribIPointer(BONE_ID_LOCATION, 4, GL_UNSIGNED_INT, sizeof(VertexBoneData), (const GLvoid*)0);
...
string findBoneName(map<string, unsigned int> m, unsigned int v) {
	for (auto const & kv : m) {
...
ostream & operator<<(ostream & out, const Matrix4f & mat) {
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			out << mat.m[i][j];
			if (j < 3) out << '\t';
...
void SkinnedMesh::generate_debug_file(const char * filename) {
	
	// call to update transforms
	vector<Matrix4f> ts;
	BoneTransform(0.f, ts);
	...
	for (unsigned int i = 0; i < m_NumBones; i++) {
		out << findBoneName(m_BoneMapping, i) << endl;
		out << m_BoneInfo[i].FinalTransformation << endl;
	}
}
...
void SkinnedMesh::ReadNodeHeirarchyDebug(float AnimationTime, const aiNode* pNode, const Matrix4f& ParentTransform, vector<Matrix4f> & trace)
{
	string NodeName(pNode->mName.data);
	...
	if (m_BoneMapping.find(NodeName) != m_BoneMapping.end()) {
		uint BoneIndex = m_BoneMapping[NodeName];
		m_BoneInfo[BoneIndex].FinalTransformation = m_GlobalInverseTransform * GlobalTransformation * m_BoneInfo[BoneIndex].BoneOffset;
		trace.push_back(m_BoneInfo[BoneIndex].FinalTransformation);
	}

	for (uint i = 0; i < pNode->mNumChildren; i++) {
		ReadNodeHeirarchyDebug(AnimationTime, pNode->mChildren[i], GlobalTransformation, trace);
	}
}

/*
Matrix4f SkinnedMesh::BoneTransformDebug(float TimeInSeconds, vector<Matrix4f>& Transforms)
{
	Matrix4f Identity;
	Identity.InitIdentity();
    ...
    return ReadNodeHeirarchyDebug(AnimationTime, m_pScene->mRootNode, Identity);
